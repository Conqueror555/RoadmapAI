...
const RoadmapViewerPage: React.FC = () => {
  ...
  const handleDownloadPDF = () => {
    ...
    pdf.text(`Total Phases: ${phases.length}`, pageWidth - margin - 10, yPosition + 10, { align: 'right' });
    pdf.text(
          `Total Steps: ${phases.reduce((total: any, phase: any) => total + phase.steps.length, 0)}`,
          pageWidth - margin - 10,
          yPosition + 20,
          { align: 'right' }
    );
    ...
    const fileName = `Career_${title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_roadmap.pdf`;
    pdf.save(fileName);
  };
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-600 capitalize">
              {selectedCategory?.replace('_', ' ')} • {phases.length} Phases
            </p>
          </div>
