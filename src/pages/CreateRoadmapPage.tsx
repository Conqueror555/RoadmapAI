...
const CreateRoadmapPage: React.FC = () => {
  ...
  const handleGenerateRoadmap = async (text: string, category: Category) => {
    ...
    try {
      ...
      if (response.success && response.roadmapNodes) {
        ...
        setProjectName(response.projectName || 'Career Roadmap'); 
  ...
  const handleDownloadPDF = () => {
    ...
    pdf.text('Career Roadmap Generator', margin, 25);
    ...
    pdf.text(`Total Phases: ${currentPhases.length}`, pageWidth - margin - 10, yPosition + 10, { align: 'right' });
    pdf.text(`Total Steps: ${currentPhases.reduce((total, phase) => total + phase.steps.length, 0)}`, pageWidth - margin - 10, yPosition + 20, { align: 'right' });
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pdf.setFontSize(8);
      pdf.setFont('helvetica', 'normal');
      pdf.text('Generated by Career Roadmap Generator', margin, pageHeight - 5);
      pdf.text(`Page ${i} of ${totalPages}`, pageWidth - margin, pageHeight - 5, { align: 'right' });
    }

    const fileName = `Flowniq_${projectName.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_roadmap.pdf`;
    pdf.save(fileName);
  };
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-600 capitalize">
                {selectedCategory?.replace('_', ' ')} • {currentPhases.length} Phases
              </p>
            </div>
      ...
      <div className="pt-20 h-screen flex">
        ...
        <div className="flex-1 w-full">
          <RoadmapCanvas
            ...
            category={selectedCategory ?? 'career'}
